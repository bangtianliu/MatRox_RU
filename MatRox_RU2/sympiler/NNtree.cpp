...
namespace Sympiler{
	namespace Internal{
		void NNtree::split(const Setup &setup, std::vector<int> lids, std::vector<int> &llids, std::vector<int> &rlids, int idx)
		{
...
//						auto lc = 2*j+1;
//						auto rc = 2*j+2;
//						split(setup,lids[j],lids[lc],lids[rc],j*i);
//					}
//				}
//				else
				{
...
#pragma omp parallel for if(setup.d<300)
