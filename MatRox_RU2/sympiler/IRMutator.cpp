...
void IRMutator::visit(const ForG *op) {
    Expr min = mutate(op->min);
    Expr extent = mutate(op->extent);
    Stmt body = mutate(op->body);
    if (min.same_as(op->min) &&
        ...
        stmt = ForG::make(op->name, min, extent, op->for_type, op->device_api, body, op->flag);
    }
}

void IRMutator::visit(const Foru *op) {
    Expr min = mutate(op->min);
    Expr extent = mutate(op->extent);
    Stmt body = mutate(op->body);
    ...
    if (min.same_as(op->min) &&
        extent.same_as(op->extent) &&
        body.same_as(op->body) && stride.same_as(op->stride)) {
        stmt = op;
    } else {
        stmt = Foru::make(op->name, min, extent, op->for_type, op->device_api, body,stride);
    }
}

void IRMutator::visit(const DeclVariable *op) {
    stmt = op;
}

void IRMutator::visit(const ForTree *op){
    Expr min = mutate(op->min);
    ...
    if(min.same_as(op->min) &&
        ...
        stmt = ForTree::make(op->tname, op->lname, op->nidx, op->idx, min, max, op->for_Type, op->device_api, body, op->flag);
    }
}
...
void IRMutator::visit(const CallMKL *op){
    std::vector<Expr > new_args(op->args.size());
    std::vector<Argument > new_argums(op->argums.size());
    for (int j = 0; j < op->argums.size(); ++j) {
        new_argums[j] = op->argums[j];
    ...
    if (!changed) {
        ...
        stmt = CallMKL::make(op->name, new_args, new_argums, op->transflag);
    }
}
