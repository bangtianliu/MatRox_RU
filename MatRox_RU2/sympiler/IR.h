...
struct ForTree : public StmtNode<ForTree> {  // found bugs,
...
//    Stmt store;
    ...
    bool flag; // true: top-down; false: bottom-up
    static Stmt make(std::string tname, std::string lname, std::string nidx, Expr _idx, Expr min, Expr max, ForType for_type, DeviceAPI device_api, Stmt body, bool flag);

};

struct
Foru : public StmtNode<Foru> {
    std::string name;
    Expr min, extent;
    Expr stride;
    ForType for_type;
    ...
    static Stmt make(std::string name, Expr min, Expr extent, ForType for_type, DeviceAPI device_api,
                     Stmt body, Expr stride=Expr());
};

struct
DeclVariable : public StmtNode<DeclVariable> {
    std::string name;
...
struct CallMKL : public StmtNode<CallMKL> {
        std::string name="cblas_dgemm";
        std::vector<Expr> args;
        std::vector<Argument> argums;
        bool transflag;
//        typedef enum {BLAS, Sympiler} CallType;
//        CallType call_type;
        ...
        int value_index;
        static Stmt make(std::string name, const std::vector<Expr> &args, bool flag);

        static Stmt make(std::string name, const std::vector<Expr> &args,
                         const std::vector<Argument> &argums, bool flag );
};
