...
void IRVisitor::visit(const ForG *op) {
    op->min.accept(this);
    op->extent.accept(this);
    op->body.accept(this);
}

void IRVisitor::visit(const Foru *op) {
    op->min.accept(this);
    op->extent.accept(this);
    op->body.accept(this);
...
void IRVisitor::visit(const DeclVariable *op) {

}

void IRVisitor::visit(const ForTree *op) {
    op->min.accept(this);
    op->max.accept(this);
...
void IRVisitor::visit(const CallMKL *op) {
    for (size_t i = 0; i < op->args.size(); i++){
        op->args[i].accept(this);
    }
}
