...
Stmt Triangular::fwdSolve() {
   ...
   // s = Block::make(Free::make(rhsDense->Name()),s);
    ...
    s = For::make(Kernel::name + "f1", Add::make(L->accessCol(i0),make_one(Int(16))),
                  L->accessCol(i2),
                  ForType::Unrolled, DeviceAPI::Host, s);
...
Stmt Triangular::testcode() {
    ...
    Expr imm1 = UIntImm::make(halide_type_t(halide_type_int,32),128);

    Expr stride = IntImm::make(halide_type_t(halide_type_int,32),4);



//    s = Store::make(vsum,"sum1",i0);

//    for (int i = 1; i < unroll; ++i) {
        Expr imm2 = IntImm::make(halide_type_t(halide_type_int,32),1);
        Expr i1 = Add::make(i0,imm2);


       t = Store::make(vsum,"sum2",Add::make(vsum,i0));

//        s = Block::make(s,t);
//    }

//    s = For::make("i", imm0,imm1,ForType::Unrolled, DeviceAPI::Host, s);

   t1 = Store::make(vsum, "init", imm0);

    s = For::make("i", imm0, imm1, ForType::Unrolled, DeviceAPI::Host,t);
...
Stmt Triangular::unroll(int ur){
    VIUnroll viroll(ur);
    loweredKer = viroll.mutate(loweredKer);
    return loweredKer;
}
