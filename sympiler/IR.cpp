...
Stmt ForG::make(std::string name, Expr min, Expr extent, ForType for_type, DeviceAPI device_api, Stmt body,
                bool flag) {
    ForG *node = new ForG;
...
Stmt ForTree::make(std::string tname, std::string lname, std::string nidx, Expr _idx, Expr min, Expr max, ForType for_type, DeviceAPI device_api,
                   Stmt body, bool flag) {
    ForTree *node = new ForTree;
    ...
    Expr beg = Variable::make(halide_type_t(halide_type_int,32),"i");
    Expr imm1 = IntImm::make(halide_type_t(halide_type_int,32),1);
    Expr imm0 = IntImm::make(halide_type_t(halide_type_int,32),0);
    Expr stop = Add::make(beg, imm1);

    node->start = Pointer::make(halide_type_t(halide_type_int,32),tname,beg);
    node->stop = Pointer::make(halide_type_t(halide_type_int,32),tname,stop);

//    node->Decel = DeclVariable::make(node->nidx);
//    node->idx = Pointer::make(halide_type_t(halide_type_int,32),lname, Variable::make(halide_type_t(halide_type_int,32), "j"));

    node->for_Type = for_type;
    node->device_api = device_api;

    Expr tidx=Variable::make(halide_type_t(halide_type_int,32), node->nidx);
//    tidx= Add::make(tidx,imm0);
//    node->store = Store::make(tidx, "st1",
//                              node->idx);
	node->idx = _idx;
    node->body = body;
...
Stmt Foru::make(std::string name, Expr min, Expr extent, ForType for_type, DeviceAPI device_api, Stmt body, Expr stride) {
    Foru *node = new Foru;
    node->name = name;
    node->min = min;
    node->extent = extent;
...
Stmt DeclVariable::make(std::string name) {
    DeclVariable *node = new DeclVariable;
    node->name = name;
    return node;
}
...
Stmt CallMKL::make(std::string name, const std::vector<Expr> &args, bool flag) {
    CallMKL *node = new CallMKL;
    node->name=name;
    node->args = args;
    node->transflag = flag;
    return node;
...
Stmt CallMKL::make(std::string name, const std::vector<Expr> &args, const std::vector<Argument> &argums, bool flag) {
    CallMKL *node = new CallMKL;
    node->name=name;
    node->args = args;
    node->argums=argums;
...
template<> void StmtNode<Foru>::accept(IRVisitor *v) const { v->visit((const Foru *)this); }
template<> void StmtNode<ForG>::accept(IRVisitor *v) const { v->visit((const ForG *)this); }
template<> void StmtNode<ForTree>::accept(IRVisitor *v) const { v->visit((const ForTree *)this); }
template<> void StmtNode<DeclVariable>::accept(IRVisitor *v) const { v->visit((const DeclVariable *)this); }
...
template<> void StmtNode<CallMKL>::accept(IRVisitor *v) const { v->visit((const CallMKL *)this); }
        ...
        template<> IRNodeType StmtNode<CallMKL>::_type_info = {};
        ...
        template<> IRNodeType StmtNode<Foru>::_type_info = {};
        template<> IRNodeType StmtNode<ForG>::_type_info = {};
        template<> IRNodeType StmtNode<DeclVariable>::_type_info = {};
        template<> IRNodeType StmtNode<ForTree>::_type_info = {};
