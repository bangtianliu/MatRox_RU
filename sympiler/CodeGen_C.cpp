...
void CodeGen_C::visit(const Foru *op) {
    if (op->for_type == ForType::Parallel) {
        do_indent();
        stream << "#pragma omp parallel for\n";
    }
    ...
    std::string id_min = print_expr(op->min);
    std::string id_extent = print_expr(op->extent);
    std::string id_stride = print_expr(op->stride);
    loopParsing= false;
    do_indent();
    stream << "for (int "
           << print_name(op->name)
...
void CodeGen_C::visit(const ForG *op) {
...
void CodeGen_C::visit(const DeclVariable *op)
{
    do_indent();
    stream << "int " << op->name << ";\n";
}

void CodeGen_C::visit(const ForTree *op) {
    loopParsing=false;
    std::string id_min = print_expr(op->min);
    std::string id_max = print_expr(op->max);
    loopParsing=false;
    if(op->flag){
        stream << "for (int "
               << "i"
    ...
    open_scope();
    std::string lmin = print_expr(op->start);
    std::string lmax = print_expr(op->stop);
    stream << "for (int "
           << "j"
           << " = " << lmin
           << ";"
           ...
           << "++)\n";
    open_scope();
//    op->Decel.accept(this);
//    op->store.accept(this);
    op->body.accept(this);
    close_scope("for-j");
    close_scope("for-i");
}
...
// call MKL to perform GEMM
void CodeGen_C::visit(const CallMKL *op) {
    std::ostringstream rhs;
    std::string tmp;
    std::vector<std::string> args;

    std::string layout("CblasColMajor");
    std::string atrans;
    std::string btrans("CblasNoTrans");
    if(op->transflag){
        atrans="CblasTrans";
    }
    else {
    ...
    args.push_back(atrans);
    args.push_back(btrans);
    ...
    stream<<args[5]<<",\n";
    do_indent();
    for (size_t i = 6; i < args.size()-1; i++)
    {
        if(i%4==0) {
            stream<<"\n";
            do_indent();
