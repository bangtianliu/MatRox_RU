...
def main(filename, gfilename,sfilename):
    ...
    #
    str_com_bar = plt.bar(neg_bar_positions + 0.5, strcom, bar_width,
                          color='#994C00',
                          edgecolor='black',
                          linewidth=line_width,
                          label='STRUMPACK-compression'
                          )
    #
    str_eval_bar = plt.bar(neg_bar_positions + 0.5, streval, bar_width,
                           bottom=strcom,
                           color='#0066CC',
                           edgecolor='black',
                           #                       hatch='//',
                           label='STRUMPACK-evaluation'
                           )
    #
...
if __name__ == '__main__':
    ...
    parser.add_argument("--m", help="require the path to hssnrhs.csv from matrox. if csv file does not exists, run sbatch nrhssh 0.0", default='./hssnrhs.csv')
    parser.add_argument("--g", help="require the path to gohssnrhs.csv from gofmm. if csv file does not exists, run sbatch GOnrhsh 0.0", default='./gohssnrhs.csv')
    parser.add_argument("--s", help="require the path to stnrhs.csv from strumpack. if csv file does not exists, run sbatch stnrhsh", default='./stnrhs.csv')
    args = parser.parse_args()
    # filename = '/home/labuser/Dropbox/H2MATRICES/PPOPP/matrox/nrhs.csv'
    #     filename=sys.argv[1]
    #     ''
