...
int32_t *slen) {
 ...
 int32_t _0 = (int32_t)(4294967295);
 int32_t _1 = (int32_t)(10);
 for (int i = _1; i > _0; i--)
 {
  int32_t _2 = i + 1;
  for (int j = levelset[i]; j < levelset[_2]; j++)
  {
   int32_t _3 = (int32_t)(4294967295);
   bool _4 = lchildren[idx[j]] == _3;
   if (_4)
   {
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 
   ...
   else
   {
    ...
    &wskel[wskeloffset[idx[j]]],  slen[idx[j]]);
    int32_t _5 = slen[idx[j]] * slen[lchildren[idx[j]]];
    int32_t _6 = _5 + VTptr[idx[j]];
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 
    slen[idx[j]],nrhs,slen[rchildren[idx[j]]],
    float_from_bits(1065353216 /* 1 */), &VT[_6], 
    slen[idx[j]], &wskel[wskeloffset[rchildren[idx[j]]]], slen[rchildren[idx[j]]], float_from_bits(1065353216 /* 1 */), 
    &wskel[wskeloffset[idx[j]]],  slen[idx[j]]);
   } // if _4 else
  } // for j
 } // for i
 uint32_t _7 = (uint32_t)(1);
 uint32_t _8 = (uint32_t)(201);
 for (int i = _7; i < _8; i++)
 {
  uint32_t _9 = (uint32_t)(1);
  int32_t _10 = i - _9;
  int32_t _11 = i + _9;
  int32_t _12 = i & 1;
  uint32_t _13 = (uint32_t)(0);
  bool _14 = _12 == _13;
  int32_t _15 = (int32_t)(_14 ? _10 : _11);
  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 
  slen[i],nrhs,slen[_15],
  _9, &B[Bptr[_10]], 
  slen[i], &wskel[wskeloffset[_15]], slen[_15], _13, 
  &uskel[uskeloffset[i]],  slen[i]);
 } // for i
 uint32_t _16 = (uint32_t)(0);
 uint32_t _17 = (uint32_t)(11);
 for (int i = _16; i < _17; i++)
 {
  int32_t _18 = i + 1;
  for (int j = levelset[i]; j < levelset[_18]; j++)
  {
   int32_t _19 = (int32_t)(4294967295);
   bool _20 = lchildren[idx[j]] == _19;
   if (_20)
   {
   ...
   else
   {
    ...
    int32_t _21 = slen[idx[j]] * slen[lchildren[idx[j]]];
    int32_t _22 = _21 + VTptr[idx[j]];
    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, 
    slen[rchildren[idx[j]]],nrhs,slen[idx[j]],
    float_from_bits(1065353216 /* 1 */), &VT[_22], 
    slen[idx[j]], &uskel[uskeloffset[idx[j]]], slen[idx[j]], float_from_bits(1065353216 /* 1 */), 
